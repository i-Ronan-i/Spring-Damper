...
def create_initial(pop_num, pop, kd_min, kd_max, kp_min, kp_max, ki_min, ki_max, desired_x, system_force):
    ...
    for s in range(pop_num):
        ...
        while flag == False:  
            ...
            #Force contraint equation
            pid_force = kd_cur*2*max(desired_x)/0.1 + kp_cur*max(desired_x)*0.9 + ki_cur*2*8/(0.1**2)
...
def crossover(a, b, system_force, desired_x):
    ...
    pid_force = new_a[0]*2*max(desired_x)/0.1 + new_a[1]*max(desired_x)*0.9 + new_a[2]*2*8/(0.1**2)
    if system_force < pid_force :
        new_a = a
    return new_a
    ...
    for i in range(0, len(pop_curr)):
        for o in range(3) :
            if random.random() < mut_prob:
                ...
                pid_force = pop_curr[i][0]*2*max(desired_x)/0.1 + pop_curr[i][1]*max(desired_x)*0.9 + pop_curr[i][2]*2*8/(0.1**2)   
                while system_force < pid_force :
                    pop_curr[i][0] = pop_curr[i][0] * 0.95
                    pop_curr[i][1] = pop_curr[i][1] * 0.95
                    pop_curr[i][2] = pop_curr[i][2] * 0.95
                    pid_force = pop_curr[i][0]*2*max(desired_x)/0.1 + pop_curr[i][1]*max(desired_x)*0.9 + pop_curr[i][2]*2*8/(0.1**2)
    return pop_curr
...
def create_next_generation(pop, pop_num, fit_val, mut_prob, kd_min, kd_max, kp_min, kp_max, ki_min, ki_max, desired_x, system_force):
    ...
    pop_cross = []
    for n in range(25):
        new_pop1 = crossover(pop[n], pop[n+1], system_force, desired_x)
...
print("Force value is: ", pop[0][0]*2*max(desired_x)/0.1 + pop[0][1]*max(desired_x)*0.9 + pop[0][2]*2*8/(0.1**2))
